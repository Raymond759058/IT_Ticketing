...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name !== '') {
        ...
        logAudit('Saved category', $name);
    }
    redirect('admin/categories.php');
}

if (isset($_GET['toggle']) && hash_equals($_SESSION['csrf_token'] ?? '', $_GET['csrf'] ?? '')) {
    $pdo->prepare("UPDATE categories SET status = 1 - status WHERE id = ?")->execute([$_GET['toggle']]);
    redirect('admin/categories.php');
}
...
$categories = $pdo->query("SELECT d.*, (SELECT COUNT(*) FROM tickets t WHERE t.category_id=d.id) ticket_count FROM categories d ORDER BY d.name")->fetchAll();
...
<div class="main-content flex-grow-1">
  <div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#deptModal" onclick="resetForm()"><i class="bi bi-plus-circle"></i> Add Category</button>
  </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <form method="POST">
        ...
        <div class="modal-header"><h5 class="modal-title" id="c_title">Add Category</h5><button class="btn-close" data-bs-dismiss="modal"></button></div>
        <div class="modal-body">
          <div class="mb-3"><label class="form-label">Name</label><input type="text" name="name" id="c_name" class="form-control" required></div>
          <div class="mb-3"><label class="form-label">Description</label><textarea name="description" id="c_desc" class="form-control"></textarea></div>
        </div>
...
<script>
function resetForm(){document.getElementById('c_title').textContent='Add Category';document.getElementById('c_id').value='';document.getElementById('c_name').value='';document.getElementById('c_desc').value='';}
function editModal(d){document.getElementById('c_title').textContent='Edit Category';document.getElementById('c_id').value=d.id;document.getElementById('c_name').value=d.name;document.getElementById('c_desc').value=d.description||'';new bootstrap.Modal(document.getElementById('deptModal')).show();}
</script>
